...
export default function HistoryScreen() {
  ...
  const fetchAlerts = async () => {
    try {
      ...
      const alertsData = Array.isArray(response.data) ? response.data : [];
      console.log("Alerts Data:", alertsData); // Log the data to verify ubicacion
  ...
  const renderAlertItem = ({ item }) => {
    console.log("Alarma Item:", JSON.stringify(item, null, 2)); // Log the item to verify ubicaciones
